...
suite "ChunkedArray - Different Types":
  ...
  test "int8 chunked array":
    let chunks = [
      newArray[int8](@[1'i8, 2]),
      newArray[int8](@[3'i8, 4, 5])
    ]
    let chunkedArray = newChunkedArray(chunks)
  ...
  test "int16 chunked array":
    let chunks = [
      newArray[int16](@[100'i16, 200]),
      newArray[int16](@[300'i16])
    ]
    let chunkedArray = newChunkedArray(chunks)
    check chunkedArray.len() == 3
  ...
  test "int64 chunked array":
    let chunks = [
      newArray[int64](@[1000000'i64, 2000000])
    ]
    let chunkedArray = newChunkedArray(chunks)
  ...
  test "float32 chunked array":
    let chunks = [
      newArray[float32](@[1.5'f32, 2.5]),
      newArray[float32](@[3.5'f32])
    ]
    let chunkedArray = newChunkedArray(chunks)
    check chunkedArray.len() == 3
