...
import unittest2
import testfixture
import ../src/narrow/[core/ffi, core/debug, compute/expressions, types/gtypes, column/primitive, tabular/table, tabular/batch, column/metadata, io/parquet]
...
suite "Push-Down Filtering":
  ...
  test "statistics to expression":
    ...
    let foo = getRowGroupPrivateOwner(rowGroup.toPtr)
    dumpObject(foo)
    g_object_unref(foo)
    dumpObject(foo)
    # echo repr foo
