...
suite "Acero - Aggregate Table":
  test "global sum aggregation without group by":
    ...
    let res = aggregateTable(table, [], [
      (field: "amount", fn: "sum", output: "total"),
    ])

    unittest2.check res.nRows == 1
    unittest2.check res.nColumns == 1
    unittest2.check res.schema[0].name == "total"
  ...
  test "single-key group by with sum":
    ...
    let res = aggregateTable(table, ["category"], [
      (field: "amount", fn: "sum", output: "total"),
    ])

    unittest2.check res.nRows == 2
    unittest2.check res.nColumns == 2
  ...
  test "fluent groupBy aggregate API":
    ...
    let res = table.groupBy("category").aggregate([
      (field: "amount", fn: "sum", output: "total"),
    ])
  ...
  test "multiple aggregations on same field":
    ...
    unittest2.check res.nRows == 2
    unittest2.check res.nColumns == 3
