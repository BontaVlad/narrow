...
suite "Timestamp - Creation and Timezone":
  ...
  test "Create Timestamp from value":
    let ts = newTimestamp(1000'i64, tuMilli, "UTC")
    check ts.unit == tuMilli
    check ts.tz == "UTC"

  test "Create Timestamp from DateTime":
    let dt = dateTime(2021, mJan, 1)
    let ts = newTimestamp(dt, tuNano, "UTC")
    let dt2 = ts.toDateTime()
    check dt2.year in [2020, 2021]

  test "Timestamp with different time units":
    let ts1 = newTimestamp(1000'i64, tuSecond, "UTC")
    let ts2 = newTimestamp(1000000'i64, tuMilli, "UTC")
    let ts3 = newTimestamp(1000000000'i64, tuMicro, "UTC")
    check ts1.unit == tuSecond
    check ts2.unit == tuMilli
    check ts3.unit == tuMicro

  test "Timestamp with different timezones":
    let ts1 = newTimestamp(1000'i64, tuNano, "UTC")
    let ts2 = newTimestamp(1000'i64, tuNano, "EST")
    check ts1.tz == "UTC"
    check ts2.tz == "EST"

  test "Timestamp string representation":
    let ts = newTimestamp(0'i64, tuNano, "UTC")
    let str = $ts
...
suite "Duration - Creation and Operations":
  ...
  test "Create Duration from value":
    let d = newDuration(5000'i64, tuMilli)
    check d.value == 5000'i64
    check d.unit == tuMilli
  ...
  test "Duration string representation":
    let d = newDuration(1000000000'i64, tuNano)
    let str = $d
  ...
  test "Duration with different units":
    ...
    let d2 = newDuration(1000'i64, tuMilli)
    let d3 = newDuration(1000000'i64, tuMicro)
    let d4 = newDuration(1000000000'i64, tuNano)
...
suite "TimestampArray - Building and Operations":
  ...
  test "Create TimestampArrayBuilder":
    let builder = newTimestampArrayBuilder(tuNano, "UTC")
    check builder.unit == tuNano
    check builder.tz == "UTC"

  test "Append to TimestampArray":
    var builder = newTimestampArrayBuilder(tuMilli, "UTC")
    let ts = newTimestamp(1000'i64, tuMilli, "UTC")
    builder.append(ts)
    builder.append(ts)
  ...
  test "Append Option to TimestampArray":
    var builder = newTimestampArrayBuilder(tuNano, "UTC")
    let ts = newTimestamp(1000'i64, tuNano, "UTC")
  ...
  test "TimestampArray timezone mismatch error":
    var builder = newTimestampArrayBuilder(tuMilli, "UTC")
    let ts = newTimestamp(1000'i64, tuMilli, "EST")
    expect(ValueError):
      builder.append(ts)
...
suite "Error Handling - Temporal Types":
  ...
  test "Null value access":
    var builder = newTimestampArrayBuilder(tuMilli, "UTC")
    builder.append(some(newTimestamp(1000'i64, tuMilli, "UTC")))
    builder.append(none(Timestamp))
    let arr = builder.finish()
