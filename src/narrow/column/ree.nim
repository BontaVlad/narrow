...
proc newRunEndEncodedDataType*(
  ...
  result.handle = garrow_run_end_encoded_data_type_new(
    runEndType.handle, valueType.handle)
  if isNil(result.handle):
    raise newException(OperationError,
      "Error creating run-end encoded data type")
...
proc newRunEndEncodedArray*[T, U](
    dataType: RunEndEncodedDataType, logicalLength: int64,
    runEnds: Array[T], values: Array[U], logicalOffset: int64 = 0
): RunEndEncodedArray =
  result.handle = verify garrow_run_end_encoded_array_new(
    cast[ptr GArrowDataType](dataType.handle), logicalLength,
    cast[ptr GArrowArray](runEnds.toPtr),
    cast[ptr GArrowArray](values.toPtr), logicalOffset)
  if isNil(result.handle):
    raise newException(OperationError,
      "Error creating run-end encoded array")
